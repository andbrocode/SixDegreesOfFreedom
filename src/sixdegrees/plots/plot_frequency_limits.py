...
def plot_frequency_limits(freq_results: Dict[str, Any], 
    ...
    # remove zeros from max_projections and min_projections
    max_projections = np.where(max_projections > 0, max_projections, np.nan)
    min_projections = np.where(min_projections > 0, min_projections, np.nan)
    ...
    # Left subplot (fmin)
    ax1.set_title('Minimum Frequencies', pad=20, fontsize=12, fontweight='bold')
    ...
    # Set rlabel position
    ax1.set_rlabel_position(ylabel_angle)
    
    # Right subplot (fmax)
    ax2.set_title('Maximum Frequencies', pad=20, fontsize=12, fontweight='bold')
    ...
    # Set rlabel position
    ax2.set_rlabel_position(ylabel_angle)
